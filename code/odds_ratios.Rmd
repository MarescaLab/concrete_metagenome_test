...
fisher <- joined_kegg %>% 
  ...
  mutate(fisher = list(fisher.test(matrix(c(meta_in,n_in,meta_out,n_out),nrow=2,ncol=2))),
         fisher.p = fisher[[1]]$p.value,
         fisher.OR = fisher[[1]]$estimate,
         fisher.ci.lower = fisher[[1]]$conf.int[1],
         fisher.ci.upper = fisher[[1]]$conf.int[2]) %>% 
  select(sample, level, group, fisher, fisher.p, fisher.OR, fisher.ci.lower, fisher.ci.upper, OR) %>% 
  unique() %>% 
...
fisher %>% 
  filter(fisher.OR > 1) %>% 
  ggplot(aes(fisher.OR, reorder(L2,fisher.OR), color = p.fdr < 0.05)) +
  geom_point() +
