...
uniref_to_kegg <- read_tsv("C:/Users/eande/Downloads/full_mapping_v201901/map_ko_uniref90.txt.gz",col_names = FALSE) %>% 
  pivot_longer(-X1,values_to = "uniref") %>% 
  select(KO = "X1",uniref)


uniref_to_kegg <- read_tsv("C:/Users/eande/Downloads/full_mapping_v201901/map_uniref90_name.txt.bz2",col_names = FALSE,n_max = 2000) #%>% 
  pivot_longer(-X1,values_to = "uniref") %>% 
  select(KO = "X1",uniref)
...
humann_for_OR <- ko_humann_no_tax %>% 
  select(id = KO, meta_abund = "S3_Fallen_humann_Abundance-RPKs") %>% 
  distinct() %>% 
  mutate(sample = "S3_Fallen",
