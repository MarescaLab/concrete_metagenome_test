...
raw_seq_folder='data/raw_sequence_files'
...
```{bash}
pwd
mv data/raw_sequence_files/S3_Fallen_R1.fastq.gz data/raw_sequence_files/S3_Fallen_S1_R1_001.fastq.gz 
mv data/raw_sequence_files/S3_Fallen_R2.fastq.gz data/raw_sequence_files/S3_Fallen_S1_R2_001.fastq.gz
```
...
trim_galore --paired -o ../qc_sequence_files --basename S3_fallen *.fastq.gz
...
contigs <- readDNAStringSet("data/assembly_metaspades/S3_fallen_biomix/contigs.fasta")

contigs_df <- as.data.frame(contigs) %>% 
  rename(sequence = "x") %>%
  ...
  select(node = "2",
         ...
         coverage = "6",
         sequence) %>% 
  mutate(across(c(node,length,coverage),~as.numeric(.)))

contigs_df %>% ggplot(aes(length)) +
  geom_histogram() +
  scale_y_log10() +
  labs(title = "Contig length distribution (log)") +
  theme_bw()

contigs_df %>% ggplot(aes(length,coverage)) +
  geom_point() +
  scale_y_log10() +
  labs(title ="Contig length vs. coverage",
       subtitle ="Assembled with metaspades",
       y = "coverage (log)") +
  theme_bw()
